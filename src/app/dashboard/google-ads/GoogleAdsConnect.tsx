...
interface Props {
  onConnected: (customerId?: string) => void;
  alreadyConnected?: boolean;
  /** "connect" runs OAuth then opens the account picker (default).
   *  "switch" skips OAuth and opens the picker directly with the stored token. */
  mode?: "connect" | "switch";
  /** Override the trigger button styling (e.g. a compact header button). */
...
export default function GoogleAdsConnect({
  ...
  return (
    <>
      <button
        onClick={mode === "switch" ? fetchResources : startOAuth}
        disabled={phase === "popup" || phase === "fetching" || phase === "saving"}
        className={
          triggerClassName ??
          "group relative inline-flex w-full items-center justify-center gap-3 overflow-hidden rounded-2xl bg-gradient-to-r from-[#4285F4] to-[#1a73e8] px-6 py-3.5 text-sm font-bold text-white shadow-lg transition-all hover:shadow-xl disabled:opacity-60"
        }
      >
        {phase === "popup" || phase === "fetching" ? (
          <Loader2 size={triggerClassName ? 13 : 18} className="animate-spin" />
        ) : mode === "switch" ? (
          <Building2 size={13} />
        ) : (
